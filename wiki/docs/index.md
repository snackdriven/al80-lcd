...
The AL80's LCD is driven by a **separate smart display module** (the STM32F103 keyboard MCU does
not render pixels — it forwards the host's HID stream over USART3). Everything here was derived
...
`0x40/0x41/0x42` screen commands. It is the **same interface VIA uses** (VIA opcodes sit in the
`0x01–0x15` range; the LCD ops sit above them), which is why only one process can hold it. Full
