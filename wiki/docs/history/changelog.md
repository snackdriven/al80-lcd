...
## 2026-07-05 — Custom-firmware image SHEAR = UART TX jitter, not geometry
...
- **now-playing is LIVE on-device** (Spotify card via `host/nowplaying-run.mjs`) — first live
  payload proving the display sequence. Detail: [Now-playing](../how-to/now-playing.md).
...
Everything else — checksums, CRC, full command map, still-image + GIF byte-maps, view-switch, clear,
time/date, DFU (documented to avoid) — is decoded and confirmed across captures, the web JS bundle,
and the desktop Qt app.
