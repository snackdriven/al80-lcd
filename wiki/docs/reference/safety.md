...
Brick risks, the DFU sequence to avoid, and the single-opener rule.
...
- **Never reflash carelessly.** The ripple lighting firmware is the whole reason for the VIA-only
  and HID-script approach. Custom-QMK bins in `firmware/` replace it, so treat every flash as
