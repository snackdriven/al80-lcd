...
64-byte reports, report ID `0`. The keyboard MCU does **not** render pixels — it forwards the
...
!!! note "Two protocols, one interface"
    ...
    That is why one interface serves both — and why only **one process** can hold it at a time.
...
acknowledge. Send `40 00 00 07 f6 02 00 …` → receive `40 00 00 07 f6 02 55 …`. This is how writes
