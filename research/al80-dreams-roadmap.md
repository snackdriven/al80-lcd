...
> **BUILT (2026-07-10/11):** the always-on host (Tier 4's blocker, Tier 6 spine) now rotates panels
> on its own — `host/cycle-run.mjs` + `host/cycle.js`, now-playing/weather/clock on a smart dwell
> timer with alert preemption, no browser tab. Device-free tests green; on-device rotation/banding
> confirm + autostart unification are still at-desk/follow-up work. See `al80-lcd-panel-auto-cycle-
> SPARC.md` and the KB's 2026-07-10/11 section.
...
## Tier 4 — ambient dashboard (needs the always-on host — HOST BUILT 2026-07-10/11)
Now-playing album art (96×160 fits a cover) and weather ✅ **shipped**, rotating on the always-on
host (`cycle-run.mjs`). Still open: Pomodoro ring, meeting countdown, build-status badge (❌ on a
webhook), scrolling ticker. Personal: the **wedding countdown** (bot already built) — "N days until
Kayla & Patrick" through 2026-03-21. All easy to render, and now have somewhere to live: any new
panel just implements the `Panel` interface (`poll/available/render`) and joins `CYCLE_PANELS`.
...
3. **The always-on local host** — unglamorous, but it's what turns half this list from a one-time
   demo into something you actually use. Originally planned in `al80-always-on-host-SPARC.md`; the
   panel-rotation version (now-playing/weather/clock + alerts, one process, no tab) shipped
   2026-07-10/11 — `al80-lcd-panel-auto-cycle-SPARC.md`.
